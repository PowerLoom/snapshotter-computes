...
class AggregateSingleAnswerProcessor(GenericProcessorAggregate):
    transformation_lambdas = None

    def __init__(self) -> None:
        self.transformation_lambdas = []
        self._logger = logger.bind(module='AggregateSingleAnswerProcessor')

    def _add_aggregate_snapshot(
        self,
        previous_aggregate_snapshot: ChainlinkOracleAggregateSnapshot,
        current_snapshot: ChainlinkOracleAnswersSnapshot,
    ):

        # build average for base snapshot epoch
        current_answer_avg = sum(current_snapshot.oracleAnswers.values()) / len(current_snapshot.oracleAnswers.values())
        sample_size = previous_aggregate_snapshot.sampleSize

        # increment rolling averages
        previous_aggregate_snapshot.averageAnswer = sample_size * previous_aggregate_snapshot.averageAnswer + current_answer_avg
        previous_aggregate_snapshot.averageAnswer /= sample_size + 1
        previous_aggregate_snapshot.averageAnswer = truncate(
            previous_aggregate_snapshot.averageAnswer,
            5,
        )

        # increment sample size
        previous_aggregate_snapshot.sampleSize += 1
    ...
    async def _calculate_from_scratch(
        ...
        tail_epoch_id, extrapolated_flag = await get_tail_epoch_id(
            redis, protocol_state_contract, anchor_rpc_helper, msg_obj.epochId, SECONDS_IN_DAY, msg_obj.projectId,
        )
        ...
        aggregate_snapshot = ChainlinkOracleAggregateSnapshot.parse_obj({'epochId': msg_obj.epochId})
        ...
        for snapshot_data in snapshots_data:
            if snapshot_data:
                snapshot = ChainlinkOracleAnswersSnapshot.parse_obj(snapshot_data)
        ...
        if current_epoch_underlying_data:
            current_snapshot = ChainlinkOracleAnswersSnapshot.parse_obj(current_epoch_underlying_data)
            aggregate_snapshot = self._add_aggregate_snapshot(
                aggregate_snapshot, current_snapshot,
            )
            aggregate_snapshot.timestamp = current_snapshot.timestamp

        self._logger.debug(f'calculate_from_scratch final sample size: {aggregate_snapshot.sampleSize}')
    ...
    async def compute(
        ...
        self._logger.info(f'Calculating 24h answer avg for {msg_obj}')
        ...
        if project_first_epoch == 0:
            ...
            aggregate_snapshot = ChainlinkOracleAggregateSnapshot.parse_obj(project_last_finalized_data)
            ...
            for snapshot_data in base_snapshots:
                if snapshot_data:
                    ...
                    aggregate_snapshot.timestamp = snapshot.timestamp
                    self._logger.debug(f'added 1 to {project_id} 24h sample size: {aggregate_snapshot.sampleSize}')

            # Remove from tail if needed
            tail_epochs_to_remove = []
            for epoch_id in range(project_last_finalized_epoch, msg_obj.epochId):
                tail_epoch_id, extrapolated_flag = await get_tail_epoch_id(
                    redis, protocol_state_contract, anchor_rpc_helper, epoch_id, SECONDS_IN_DAY, msg_obj.projectId,
                )
            ...
            if tail_epochs_to_remove:
                ...
                for snapshot_data in tail_epoch_snapshots:
                    if snapshot_data:
                        ...
                        self._logger.debug(
                            f'removed 1 from {project_id} 24h sample size: {aggregate_snapshot.sampleSize}',
                        )

            self._logger.debug(f'Final 24h sample size for {project_id}: {aggregate_snapshot.sampleSize}')
